...
The Dec-POMDP state representation (*z*) in MPDF is the introspective confidence generated by a critic model. The optimization objective uses a KL-regularized rank-matching loss to ensure that the policy stays within a trust region of its supervised initialization while maximizing the expected team-level return.   
